...
class BlackjackGame:
    ...
    def player_split(self):
        if self.player.can_split():
            if self.balance >= self.bet:
                ...
                self.player.split()
                self.player.add_card(self.deck.draw())  # Tirer sur main 1
                self.player.add_card(self.deck.draw(), split=True)  # Tirer sur main 2
                self.current_hand = "main"
            else:
    ...
    def stand(self):
        # Si on est sur la main principale et qu'il y a une main split, passer à la split
    ...
    def evaluate(self):
        ...
        # Évaluer la main split si elle existe
